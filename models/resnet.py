...
def resnet34(num_classes, pretrained_model=None):
    net = ResNet(BasicBlock, [3, 4, 6, 3], num_classes=num_classes)
    if pretrained_model:
        net.load_pretrained_model(torch.load(pretrained_model))
    return net

def resnet50(num_classes, pretrained_model=None):
    net = ResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes)
    if pretrained_model:
        net.load_pretrained_model(torch.load(pretrained_model))
    return net

def resnet50_32x4d(num_classes, pretrained_model=None):
    net = ResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, groups=32, width_per_group=4)
    if pretrained_model:
        net.load_pretrained_model(torch.load(pretrained_model))
    return net